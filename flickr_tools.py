...
class FlickrTools:
    ...
    def albums(self):
        result = self.flickr.photosets.getList()
        return [(a.attrib["id"], self.album_name(a.attrib["id"])) for a in result[0]]

    def album_name(self, album_id):
        result = self.flickr.photosets.getInfo(photoset_id =album_id)
